...
if (typeof window !== 'undefined' && typeof document !== 'undefined') {
  ...
  // (o redimensionamento com animação apagava os canvas no momento da impressão).
  const pageLens = {};
  let printing = false;
  const preparePrint = () => {
    ...
    Object.values(charts).forEach(c => {
      try {
        ...
        c.canvas.parentNode.appendChild(img);
      } catch (e) { /* gráfico indisponível */ }
    });
  };
  const restorePrint = () => {
    if (!printing) return; printing = false;
    $$('.print-img').forEach(i => i.remove());
  ...
  if (window.matchMedia) {
    ...
    const onMq = e => (e.matches ? preparePrint() : restorePrint());
